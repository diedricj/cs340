module.exports = function(){
    ...
    function getPokemon(res, mysql, context, complete){
        ...
        mysql.pool.query("SELECT pokedex.id, pokedex.pokemon_name, pokedex.regional, pokedex.shiny, pokedex.special FROM pokedex", function(error, results, fields){
            if(error){
    ...
    function getAPokemon(res, mysql, context, id, complete){
        //mysql.pool.query("SELECT bsg_people.character_id as id, fname, lname, bsg_planets.name AS homeworld, age FROM bsg_people INNER JOIN bsg_planets ON homeworld = bsg_planets.planet_id", function(error, results, fields){
        mysql.pool.query("SELECT pokedex.id, pokedex.pokemon_name, pokedex.regional, pokedex.shiny, pokedex.special FROM pokedex WHERE id = '" + id + "'", function(error, results, fields){
            if(error){
                res.write(JSON.stringify(error));
                res.end();
            }
    ...
    router.post('/', function(req, res){
        ...
        console.log(req.body)
        var mysql = req.app.get('mysql');
        var sql = "INSERT INTO pokedex (pokemon_name, shiny, regional, special) VALUES ('" + req.body.pokemon_name + "', " + req.body.regional + ", " + req.body.shiny + ", " + req.body.special + ");";
         console.log(sql);
